<template>
  <div>
    <a-table size="middle" bordered :columns="columns" :data-source="dataSource" :loading="loading" @expand="handleExpand" :expandIcon="customExpandIcon" :pagination="false">
  ...
  methods: {
    ...
    loadData() {
      ...
      if (this.currentId != id) {
        this.currentId = id;
        this.loading = true;
        getAction(this.url.list, { ptypeId: id })
          .then((res) => {
            if (res.success) {
    ...
    innerLoadData(record) {
      this.loading = true;
      getAction(this.url.innerlist, { ptypeId: record.ptypeId, procId: record.procId })
        .then((res) => {
          if (res.success) {
